  ...
  slate.Variants = (function() {
    ...
    Variants.prototype = $.extend({}, Variants.prototype, {
        ...
        var currentOptions = $.map($(this.singleOptionSelector, this.$container), function(element) {
          ...
          if (type === 'radio' || type === 'checkbox') {
            if ($element[0].checked) {
              ...
              currentOption.id = $element.data('id');
      ...
      _updateImages: function(variant) {
        ...
        var items = Array.from(document.getElementsByClassName("variant-image"));
        items.forEach(item=>{
          item.style.display = 'none';
        })
		
        const currentImages = Array.from(document.getElementsByClassName(`variant_${variant.id}`))
          currentImages.forEach(item=>{
            item.style.display = 'block';
          })
        
        /*
        if(variant && variant.title && variant.title &&  variant.title.toLowerCase().indexOf('white')>-1){
       ...
       _updateSubOptions: function() {
        ...
        $(".wood-option-item").hide();
        $(".wood-option-item").attr("data-shown",false);

        const woodOptionId = `option${$("#wood-option-wraper").attr("data-wood-opt-id")}`;
        const options = this.product.options;
        
        const selectedOptionValues = this._getCurrentOptions()
		this.product.variants.forEach(variant=>{
          if(selectedOptionValues.every((sov,index)=>{ 
            return sov.value == variant[`option${index + 1}`] || sov.index == woodOptionId})){
              	$(`#options_variant_${variant.id}`).show();
            	$(`#options_variant_${variant.id}`).attr("data-shown",true);
          }
		});
		
         
         const selectedHiddenOptionValue = $('.wood-option-item[data-shown=false] input.wood_variant_input:checked').val();
         
         if(selectedHiddenOptionValue){
           const shownVariant = $(`.wood-option-item[data-shown=true] input.wood_variant_input[value='${selectedHiddenOptionValue}']`);
           if(shownVariant.length>0)
           	shownVariant.prop('checked', true)
           else
             $(`.wood-option-item[data-shown=true] input.wood_variant_input:first-child`).prop('checked', true)
         }
      ...
      _updateHistoryState: function(variant) {
        ...
        const _productURLData = window.location.href.split("products/")
      	const _variantsData = _productURLData[1] && _productURLData[1].split("/") && _productURLData[1].split("/")[1];
        const _colorFromURL = _variantsData && _variantsData.split("#")[0];
  	    const _woodFromURL = _variantsData && _variantsData.split("#")[1];
        const _wirelessFromURL = _variantsData && _variantsData.split("#")[2];
        
        const color = (variant && variant.options && variant.options[0]) || (_colorFromURL && (_colorFromURL.toLowerCase() =="black" || _colorFromURL.toLowerCase() =="white") ? _colorFromURL : "Black");
  	  	const wood = (variant && variant.options && variant.options[1]) || (_woodFromURL || "Oak");
        const wireless = (variant && variant.options && variant.options[2]) || (_wirelessFromURL || "Optimized for iPhone 12 (MagSafe+Qi)");
        
        const variantURLData = `${color}#${wood}`;
        const pathnames = window.location.pathname.split("/");
        if(pathnames.some((name, index)=>name=="products" && index < pathnames.length-2 ))
          pathnames[pathnames.length - 1] = variantURLData;
        else
          pathnames[pathnames.length] = variantURLData;
        
        var newurl = window.location.protocol + '//' + window.location.host + pathnames.join("/");
        window.history.replaceState({path: newurl}, '', newurl);
//debugger
// console.log(variantURLData, 'variantURLData');
      },
  ...
  theme.collapsibles = (function() {
    ...
    function openFirst() {
      if ($(selectors.trigger)[0]) {
        $(selectors.trigger)[0].classList.add(classes.open);
        var $element = $(selectors.trigger)[0];
        let opendModulId = $element.getAttribute('aria-controls');
        document.querySelector(`#${opendModulId}`).classList.add('is-open');
        document.querySelector(`#${opendModulId}`).style.height = 'auto';
        isAutoHeight = $element.classList.contains(classes.autoHeight);
      }
    }
  ...
  theme.Product = (function() {
    ...
    function Product(container) {
      ...
      this.selectors = {
        ...
        comparePrice: '.product__price--compare',
        savePrice: '.product__price-savings',
        ...
        variantColorSwatch: '.variant__input--color-swatch-' + sectionId,
  		variantWoodSwatch: '.variant__input--wood-swatch-' + sectionId,
        variantWirelessSwatch: '.variant__input--wireless-swatch-' + sectionId,
    ...
    Product.prototype = $.extend({}, Product.prototype, {
      ...
      initVariants: function() {
        ...
        if ($(this.selectors.variantWoodSwatch).length) {
          $(this.selectors.variantWoodSwatch).on('change', function(evt) {

            
            
            
            var $el = $(evt.currentTarget);
            var color = $el.data('wood-name');
             var index = $el.data('wood-index');
            this.updateWoodName(color, index);
          }.bind(this));
        }
        
        if ($(this.selectors.variantWirelessSwatch).length) {
          $(this.selectors.variantWirelessSwatch).on('change', function(evt) {

			 var $el = $(evt.currentTarget);
            var color = $el.data('wireless-name');
             var index = $el.data('wireless-index');
            this.updateWirelessName(color, index);
        ...
        //colorVariationFromURL.toLowerCase()
        
        createEventForOption("Color", decodeURIComponent(colorVariationFromURL.toLowerCase()), this);
        createEventForOption("Wood base", decodeURIComponent(woodVariationFromURL.toLowerCase()), this);
        
        function createEventForOption(name,value,that){
          
          const radios = document.getElementsByName(name);
          let element;
          for (var j = 0; j < radios.length; j++) {
            if (radios[j].value && radios[j].value.toLowerCase().indexOf(value) > -1) {
                element = radios[j];
          ...
          if(document.createEvent){
              if(element){
                try{
                  ...
                  event.variant = that.variants.currentVariant;
              ...
              varevent.initEvent("variantChange", true, true);
              varevent.eventName = "variantChange";
              varevent.variant = that.variants.currentVariant;
              that.$container.trigger({
                type: 'variantChange',
                variant: that.variants.currentVariant
              });
      ...
      updateWoodName: function(wood, index) {
        // Updates on radio button change, not variant.js
        $('#VariantWoodLabel-' + this.sectionId + '-' + index).text(wood);
      },
      
      updateWirelessName: function(wireless, index) {
        // Updates on radio button change, not variant.js
        $('#VariantWirelessLabel-' + this.sectionId + '-' + index).text(wireless);
      },
        ...
        var $swatches = this.$container.find(selectors.colorSwatch);
         ...
         $swatches.on({
          click: function (evt) {
            ...
            const varid = $el.data('variant-id');
            const varname = $el.data('variant-name');
            const prodid = $el.data('product-id');
            const variantPrice = $el.data('variant-price');
            const variantCompareAtPrice = $el.data('variant-compare-at-price');
            const image = $el.data('variant-image');

            const link = $(`#product_link-${prodid}`).attr('href').split("/");
            
            const priceElement = $(`#product_price--${prodid}`);
            const comparePriceElement = $(`#product_price--compare--${prodid}`);
			
            
            const variantInput = $(`#variant-id-input_${prodid}`);
            variantInput.val(varid);
            
            const prodWrapperElement = $(`#prod-grid-wrapper-${prodid}`);
            if(variantCompareAtPrice > variantPrice){
              prodWrapperElement.addClass("prod-on-sale");
            }
            else{
              prodWrapperElement.removeClass("prod-on-sale");
            }
              
           if(varname.toLowerCase().indexOf("white") > -1){
              $(`#variant-hover--${prodid}-black`).hide();
              $(`#variant-hover--${prodid}-white`).show();
             $(`#variant-hover--${prodid}-navy-alcantara-darkblue`).hide();
              link[link.length - 1] = "White";
            }
            else if(varname.toLowerCase().indexOf("blue") > -1){
              $(`#variant-hover--${prodid}-black`).hide();
              $(`#variant-hover--${prodid}-white`).hide();
              $(`#variant-hover--${prodid}-navy-alcantara-darkblue`).show();
              link[link.length - 1] = "Navy alcantara darkblue";
            }
            else{
              $(`#variant-hover--${prodid}-black`).show();
              $(`#variant-hover--${prodid}-white`).hide();
              $(`#variant-hover--${prodid}-navy-alcantara-darkblue`).hide();
              link[link.length - 1] = "Black";
            }
            ...
            $(`#product_link-${prodid}`).attr('href', newlink);
            
            priceElement.text(variantPrice);
            comparePriceElement.text(variantCompareAtPrice)
  ...
  theme.HeaderSection = (function() {
    ...
    const dropdownNavLinks = document.querySelectorAll('.site-nav--has-dropdown');
    const navDropDown = document.querySelectorAll('.site-nav__dropdown');
    for (const [index, dropdownNavLink] of Array.from(dropdownNavLinks).entries()) {
      dropdownNavLink.addEventListener("mouseover", function(e) {
        e.preventDefault();
        this.querySelector('.site-nav__link').classList.add('is-active');
        this.querySelector('.site-nav__dropdown-icon').classList.add('is-open-nav');
      });

      dropdownNavLinks[index].addEventListener('mouseout', function(e) {
        if (e.target !== navDropDown[index]) {
          this.querySelector('.site-nav__link').classList.remove('is-active');
          this.querySelector('.site-nav__dropdown-icon').classList.remove('is-open-nav');
        }
      })

      navDropDown[index].addEventListener("mouseover", function(e) {
        const isActiveItems = document.querySelectorAll('.site-nav--has-dropdown .site-nav__link');
        const navIconToggles = document.querySelectorAll('.site-nav--has-dropdown .site-nav__dropdown-icon');

        isActiveItems[index].classList.add('is-active');
        navIconToggles[index].classList.add('is-open-nav');
      })

      navDropDown[index].addEventListener('mouseout', (e) => {
        const isActiveItems = document.querySelectorAll('.is-active');
        const navIconToggles = document.querySelectorAll('.site-nav__dropdown-icon');

        [...navIconToggles].forEach((navIcon) => navIcon.classList.contains('is-open-nav') && navIcon.classList.remove('is-open-nav'));
        [...isActiveItems].forEach((isActiveItem) => isActiveItem.classList.contains('is-active') && isActiveItem.classList.remove('is-active'));
      })
    }
    ...
    function Header(container) {
      ...
      const mobileNavItems = document.querySelectorAll('.mobile-nav__has-sublist');
      for (const mobileNavItem of Array.from(mobileNavItems)) {
        mobileNavItem.addEventListener('click', function(e) {
          e.preventDefault();
          this.nextElementSibling.classList.toggle('is-nav-closed');
          this.nextElementSibling.classList.contains('is-nav-closed') ?
          this.querySelector('.site-nav__dropdown-icon').classList.remove('is-open-nav') :
          this.querySelector('.site-nav__dropdown-icon').classList.add('is-open-nav');
        })
      }
...
//////////JS ADDED BY BN//////////
const navItems = document.querySelectorAll('.site-nav__link--underline');
...
for (const navItem of navItems) {
  ...
  if (pageHref === navItem.getAttribute('href')) {
...
const variantsButtons = document.querySelectorAll('.variant__button-label');
let sliceSlideIndex;
if (variantsButtons) {
  for (const variantButton of variantsButtons) {
    variantButton.addEventListener('click', function(e){
      sliceSlideIndex = this.dataset.id;

      if (sliceSlideIndex) {
        $('.carousel-main').slick('slickGoTo', +sliceSlideIndex);
        $('.carousel-main-gift-card').slick('slickGoTo', +sliceSlideIndex);
      }
    })
  }
}
...
function slickSliderController() {
  ...
  var sliderCall = {
    photoGallery: function() {
        var slider = $(".carousel-main");
        slider.slick({
          slidesToShow: 1,
          slidesToScroll: 1,
          arrows: true,
          accessibility: false,
          nextArrow: '<button class="slick-button slick-prev-next-button next"><svg xmlns="http://www.w3.org/2000/svg" width="11" height="17.802" viewBox="0 0 11 17.802"><path id="right" d="M51.006,0l-2.1,2.1,6.8,6.8-6.8,6.8,2.1,2.1,8.9-8.9Z" transform="translate(-48.907)" fill="#101010"/></svg></button>',
          prevArrow: '<button class="slick-button slick-prev-next-button previous"><svg xmlns="http://www.w3.org/2000/svg" width="11" height="17.802" viewBox="0 0 11 17.802"><path id="left" d="M57.808,0l2.1,2.1-6.8,6.8,6.8,6.8-2.1,2.1-8.9-8.9Z" transform="translate(-48.907)" fill="#101010"/></svg></button>',
          asNavFor: '.carousel-nav',
        });

        var slider = $(".carousel-main-gift-card");
        slider.slick({
          slidesToShow: 1,
          slidesToScroll: 1,
          arrows: false,
  ...
  $('.carousel-nav').slick({
    ...
    dots: false,
    arrows: true,
    nextArrow: '<button class="arrow-thumbnail"><svg xmlns="http://www.w3.org/2000/svg" width="11" height="17.802" viewBox="0 0 11 17.802"><path id="right" d="M51.006,0l-2.1,2.1,6.8,6.8-6.8,6.8,2.1,2.1,8.9-8.9Z" transform="translate(-48.907)" fill="#101010"/></svg></button>',
    prevArrow: '<button class="arrow-thumbnail"><svg xmlns="http://www.w3.org/2000/svg" width="11" height="17.802" viewBox="0 0 11 17.802"><path id="left" d="M57.808,0l2.1,2.1-6.8,6.8,6.8,6.8-2.1,2.1-8.9-8.9Z" transform="translate(-48.907)" fill="#101010"/></svg></button>',
    infinite: true,
  ...
  $('.carousel-main').on('afterChange', function(event, slick, currentSlide, nextSlide){
    const link = document.querySelector(`div[data-slick-index="${currentSlide}"]`);
      $('div[data-slick-index]').removeClass('slick-current');
      link?.classList?.add('slick-current');
  });
}

function recommendataionCaruselsController() {
  if (document.body.clientWidth < 768) {
    const gridItems = document.querySelectorAll('.product-grid-item-rec');
    for (let gridItem of gridItems) {
      gridItem.classList.remove('medium-up--one-quarter');
      gridItem.classList.remove('small--one-half');
      gridItem.classList.add('medium-up--one-half');
    }
  ...
  let target = document.querySelector('.product-recommendations');
  const config = {
      attributes: true,
      childList: true,
      subtree: true
...
function makeSoldOutButtonDisabled() {
  const soldOut = document.querySelector('.btn-add-to-cart');
  if (!soldOut) return;

  if (soldOut.classList.contains('sold-out')) {
    soldOut.disabled = true;
  }
}

let isCordSelected = false;
let variantId;
if (window.location.search.includes('variant')) {
  variantId = window.location.search.split('=')[1];
} else {
  variantId = document.querySelector('[data-variant-id]') && document.querySelector('[data-variant-id]').getAttribute('data-variant-id');
}

function init() {
  makeSoldOutButtonDisabled();
  generateYouMayAlsoLikeProducts();
  addFadeToTexts();
  currnetProductColorCheck();
  recommendataionCaruselsController();
  getProductId();

  theme.collapsibles.openFirst();
  checkCordSelection();

  const cordSelectionContainer = document.querySelector('.product_cord-select-container');
  cordSelectionContainer && cordSelectionContainer.addEventListener('click', function(e) {
    cordSelectionController(e);
  });

  const productCordVariants = document.querySelectorAll('.product-select-variant');
  if (productCordVariants.length) {
    for (productCordVariant of productCordVariants) {
      productCordVariant.addEventListener('click', function(e) {
        selectCord(e);
      })
    }
  }
...
function cordSelectionController(e) {
  const cordType = localStorage.getItem('selectedCord');
  const cordVariants = e.currentTarget.querySelector('.product-select-variants');
  e.currentTarget.classList.toggle('product_cord-select-active');
  cordVariants.classList.toggle('product_select-variants-visibale');
}

function checkCordSelection() {
  const selectOptionSelectedValue = document.querySelector('.product_select-header');
  if (!selectOptionSelectedValue) return;
  const cordType = localStorage.getItem('selectedCord');
  selectOptionSelectedValue.innerHTML = cordType ? cordType : 'Select a cord type';
}

function cordValidation() {
  const cordType = localStorage.getItem('selectedCord');
  const cordSelectionWrapper = document.querySelector('.product_cord-select-container');

  if (!cordSelectionWrapper) return;

  const cordSelectionErrorMessage = document.querySelector('.product_select_cord');
  const addToCartButton = document.querySelector('.btn-add-to-cart');
  const buyNowBtn = document.querySelector('.shopify-payment-button__now');

  if (!cordType) {
    cordSelectionErrorMessage.classList.add('select-error_show');
    cordSelectionWrapper.classList.add('product_cord-select-error');
    addToCartButton.setAttribute('disabled', true);
    buyNowBtn && buyNowBtn.setAttribute('disabled', true);
  } else {
    cordSelectionErrorMessage.classList.remove('select-error_show');
    cordSelectionWrapper.classList.remove('product_cord-select-error');
    !addToCartButton.classList.contains('disabled') && addToCartButton.removeAttribute('disabled');
    buyNowBtn && !buyNowBtn.classList.contains('disabled') && buyNowBtn.removeAttribute('disabled');
  }
}

function currnetProductColorCheck() {
  const pathname = window.location.pathname;
  const productColorItems = document.querySelectorAll('.product__color-item');

  for (const productColorItem of productColorItems) {
    if (pathname.includes(productColorItem.dataset.handle)) {
      productColorItem.querySelector('.product__color-img').classList.add('active_image');
      productColorItem.querySelector('.product__color-variant-label').classList.add('active_label');
    };
  }
}
...
function selectCord(e) {
  let target = e.target;
  isCordSelected = true;
  localStorage.setItem('selectedCord', target.innerHTML);
}

function quantityInputController() {
  const inputPlusButton = document.querySelector('.js-qty__adjust--plus');
  const inputMinusButton = document.querySelector('.js-qty__adjust--minus');
  const quantityInput = document.querySelector('.js-qty__num');
  const quantityCurrent = quantityInput && quantityInput.getAttribute('data-quantity');
  const errorMsg = document.querySelector('.product_error_msg');
  inputPlusButton && inputPlusButton.addEventListener('click', (e) => {
    quantityInput.value = Number(quantityInput.value) + 1;
    localStorage.setItem('productQuantity', quantityInput.value);
    if (Number(Math.abs(quantityCurrent)) === Number(quantityInput.value)) {
      inputPlusButton.disabled = 'disabled';
      errorMsg.classList.remove('hide');
    }
  });

  inputMinusButton && inputMinusButton.addEventListener('click', (e) => {
    if(Number(quantityInput.value) > 1) {
      quantityInput.value = Number(quantityInput.value) - 1;
      localStorage.setItem('productQuantity', quantityInput.value);

       if (Number(Math.abs(quantityCurrent)) > Number(quantityInput.value)) {
           inputPlusButton.removeAttribute('disabled')
          errorMsg.classList.add('hide');
       }
    }
  });

  quantityInput && quantityInput.addEventListener('input', (e) => {
    if (!Number(e.data)) return;
    localStorage.setItem('productQuantity', quantityInput.value);
  });

  if (quantityInput) quantityInput.value = localStorage.getItem('productQuantity');
}
...
function addToCart (body) {
  return fetch(`${window.Shopify.routes.root}cart/add.js`, { ...fetchConfig(), ...{ body } })
    .then((response) => {
      return response.json();
    })
}

function changeCart (body) {
  return fetch(`${window.Shopify.routes.root}cart/change.js`, { ...fetchConfig(), ...{ body } })
    .then((response) => {
      return response.json();
    })
}
...
function addItemsToCart() {
  const addToCartButton = document.querySelector('.btn-add-to-cart');
  const isCordSelectionExists = document.querySelector('.product__card-container');
  const selectOptionSelectedValue = document.querySelector('.product_select-header');

  if (!addToCartButton) return;
  addToCartButton.addEventListener('click', async function(e) {
    e.preventDefault();
    const selectedValue = isCordSelectionExists ? localStorage.getItem('selectedCord') : true;
    cordValidation();

    let body = {};

    if (addToCartButton.classList.contains("preorder-btn")){
      body = {
        quantity: Number(localStorage.getItem('productQuantity')),
        id: Number(variantId),
        properties: {
          'Order Info': "This order has backordered items, fulfillment will begin in March"
        }
      }} else {
        body = {
          quantity: Number(localStorage.getItem('productQuantity')),
    ...
    if (selectedValue) {
      try {
        await addToCart(JSON.stringify( body ))
        window.location.pathname = '/cart';

        isCordSelected = false;
        localStorage.removeItem('selectedCord');
      } catch (err) {
...
function buyItNow() {
  ...
  buyNowBtn.addEventListener('click', (e) => {
    ...
    const url = `/cart/${variantId}:${localStorage.getItem('productQuantity')}`;
    const isCordExistsForValidation = isCordSelectionExists ? isCordSelected : true;
    if (isCordExistsForValidation) {
...
function generateYouMayAlsoLikeProducts() {
...
// blog page active tag
function getTheActiveTagBlog(selector, className) {
  if (!selector) return;
  
  for (const blogPostTag of selector) {
    if (blogPostTag.dataset.tagname === location.pathname) {
      blogPostTag.classList.add(className);
    }
  }
}
const blogPostsTag = document.querySelectorAll('[data-tagname]');
const howItWorksTags = document.querySelectorAll('[data-tagname]');

getTheActiveTagBlog(blogPostsTag, 'selected-blog-tag');
getTheActiveTagBlog(howItWorksTags, 'works-tab-active');
//blog page active tag end
...
function addFadeToTexts() {
  document.addEventListener("scroll", () => {
    let textsWithFade = document.querySelectorAll(".aos-init");
    for (const textWithFade of textsWithFade) {
      if (partInViewport(textWithFade)) {
        textWithFade.classList.add('aos-animate');
      } else {
        textWithFade.classList.remove('aos-animate');
      }
    }
  });
}
...
function updateCartQuantity() {
  const inputActionButtons = document.querySelectorAll('.js-qty__adjust');
  const inputMinusButton = document.querySelectorAll('.js-qty__adjust--minus');
  const quantityInputs = document.querySelectorAll('.js-qty__num');

  if (!Array.isArray(inputActionButtons) && !inputActionButtons.length) return;

  for (let index = 0; index < inputActionButtons.length; index++) {
    const dataAction = inputActionButtons[index].dataset.action;
    
    inputActionButtons[index].addEventListener('click', async function(e) {
      const productVariantLKey = e.currentTarget.parentNode.querySelector('.js-qty__num').dataset.id;
      let closestInputValue = e.currentTarget.parentNode.querySelector('.js-qty__num').value;

        if (dataAction === 'minus') {
          if(Number(closestInputValue) > 1) {
            closestInputValue = Number(closestInputValue) - 1;
          }
        }
        ...
        e.currentTarget.parentNode.querySelector('.js-qty__num').value = closestInputValue;
        
        const data = {
          id: productVariantLKey,
          quantity: Number(closestInputValue),
        }

        try {
          await changeCart(JSON.stringify(data));
          location.reload();
        } catch (error) {
          console.log('CONNECTION ERROR')
        }
  ...
  for (let index = 0; index < quantityInputs.length; index++) {
    quantityInputs[index].addEventListener('input', async function(e) {

      if (!Number(e.data) && e.data !== null) return;
